...
export class JobSchedulerService {
  ...
  async checkSoonToExpireVipJobs() {
    try {
      ...
      this.logger.log(
        `Found ${soonToExpireJobs.length} jobs with VIP expiring within 2 days`,
      );
    ...
    for (const job of expiredJobs) {
      try {
        ...
        if (!job.company.email) {
          this.logger.warn(
            `No email found for company ID: ${job.company.id}, job ID: ${job.id}`,
          );
          continue;
        }
        ...
        this.logger.log(
          `Sent VIP expiration email for job "${job.title}" (ID: ${job.id}) to ${job.company.email}`,
        );
      } catch (error) {
        this.appLogger.error(
          this.requestContext,
  ...
  private async sendExpiringWarningNotifications(soonToExpireJobs: Job[]) {
    for (const job of soonToExpireJobs) {
      try {
        ...
        this.logger.log(
          `Sent VIP expiring warning for job "${job.title}" (ID: ${job.id}) to ${job.company.email}`,
        );
      } catch (error) {
        this.appLogger.error(
          this.requestContext,
