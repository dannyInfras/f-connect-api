...
export class ConsolidatedMigration1751300000000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE "cv" ADD "url" text`);
        ...
        // 5. Create function that will run on each row update to check if vip has expired
        await queryRunner.query(`
        ...
        // 6. Create trigger that runs on each row update/insert
        await queryRunner.query(`
        ...
        // 7. Create a function for daily check of expired VIP jobs
        await queryRunner.query(`
        ...
        // 8. Try to set up pg_cron if available, but don't fail if it's not
        try {
            ...
            if (pgCronExists && pgCronExists.length > 0) {
                ...
                console.log(
                    'pg_cron extension not available. Daily VIP expiry check will not be scheduled automatically.',
                );
                console.log(
                    'You will need to set up a cron job in your application to call the daily_check_expired_vip_jobs() function daily.',
                );
            }
        } catch (error) {
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE "job" DROP COLUMN "vip_expired"`);
